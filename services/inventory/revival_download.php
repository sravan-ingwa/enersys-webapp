...
$heading = 'REVIVAL DETAILS';
	$sql = mysqli_query($mr_con,"SELECT createdDate,revival_no,wh_alias,eng_name,pdf FROM ec_material_revival WHERE item_alias ='$alias' AND flag='0' GROUP BY item_alias");
				...
				$download.="<table width='100%' class='simp'>";
					$download.="<tr>";
						$download.="<td class='col-md-5 Exp_dashboard'>";
							$download.="<h4>REVIVAL No</h4>";
							$download.="<p>".checkempty(strtolower($row['revival_no']))."</p>";
						$download.="</td>";
						$download.="<td>";
					...
					$download.="</tr>";
				$download.="</table>";
					$sql1 = mysqli_query($mr_con,"SELECT * FROM ec_material_revival WHERE item_alias ='$alias' AND flag='0'");
					if(mysqli_num_rows($sql1)){$cap_arr=array();
						$download1.='<table class="cont_3" style="width:100%"><tr><td align="left"><h3>VIEW REVIVAL</h3></td></tr></table>';
						...
						$download1.='<td>'.checkemptydash(($result=='6' ? 'In Progress' : ($result=='2' ? 'Pass':($result=='3' ? 'Fail':'NA')))).'</td>';
						$i++;}
					$download1.='</tr></table>';
					}
	...
	$filename='Revival_'.date('d-m-Y_H_i_s');
