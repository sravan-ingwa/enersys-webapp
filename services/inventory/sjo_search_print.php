...
function gd_dm_view_count($item_description,$ref){ global $mr_con; 
	...
	$sql=mysqli_query($mr_con,"SELECT * FROM ec_item_code WHERE item_code_alias='$alias' AND flag=0");
		...
		$print='<html><body><table>
				...
				$print.='<p class="single_record"></p>';
					...
					if(mysqli_num_rows($sql1)){
						...
						<tr>
							...
							<tr><th align="left">SR.NO</th><th>HISTORY</th><th align="left">TRANSACTION DATE</th></tr>';
							$i=0;while($row1=mysqli_fetch_array($sql1)){
								$print.='<tr><td>'.($i+1).'</td>';
								$print.='<td>'.checkempty($row1['message']).'</td>';
								$print.='<td>'.checkempty(dateTimeFormat($row1['transaction_date'],'d')).'</td></tr>';
							$i++;}
						$print.='</td></tr></table>';
					}
				...
				$print.="<table width='100%' class='simp'>";
					...
					$print.="<td>";
							$print.="<h4>ITEM CODE</h4>";
							$print.="<p>".checkempty(alias($row['item_code'],'ec_accessories','accessories_alias','accessory_description'))."</p>";
