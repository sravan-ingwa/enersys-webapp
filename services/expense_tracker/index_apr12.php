...
function serallowances_add(){ global $mr_con;
	...
	if($chk=='0'){
		...
		if($zone==''){$res= "Select Zone";}
		elseif($state==''){$res= "Select State";}
		elseif($district==''){$res= "Select District";}
		...
		elseif($local_conveyance==''){$res= "Enter Local Conveyance";}
			...
			$q=mysqli_query($mr_con,"SELECT id FROM ec_service_allowances WHERE zone_alias = '".$zone."' AND state_alias = '".$state."' AND district_alias = '".$district."'");
				if(mysqli_num_rows($q)=='0'){
					$sql=mysqli_query($mr_con,"INSERT INTO ec_service_allowances (zone_alias, state_alias, district_alias, lodging_amount, daily_allowance, local_conveyance, service_allowance_alias, created_date) VALUES ('".$zone."','".$state."','".$district."','".$lodging_amt."','".$daily_allowance."','".$local_conveyance."','".$alias."','".date("Y-m-d")."')");
...
function serallowances_edit(){ global $mr_con;
	...
	if($chk==0){
		...
		elseif($local_conveyance==''){$res= "Enter Local Conveyance";}
			...
			$q=mysqli_query($mr_con,"SELECT id FROM ec_service_allowances WHERE zone_alias = '".$zone."' AND state_alias = '".$state."' AND district_alias = '".$district."' AND service_allowance_alias <> '$ser_alias'");
			if(mysqli_num_rows($q)=='0'){
...
function limit_add(){ global $mr_con;
		...
		if($chk==0){
			...
			for($i=0;$i<count($_REQUEST['designation_alias']);$i++){
				...
				if($designation_alias=='0'){$res= "Please Enter Designation Name";}
				else{
...
function limit_update(){ global $mr_con;
		...
		if($chk==0){
			...
			elseif($limit_amount==''){$res= "Please Enter Limit Amount";}
				...
				$q=mysqli_query($mr_con,"SELECT id FROM ec_expense_limits WHERE designation_alias='$designation_name' AND  limit_alias<>'$limit_alias' AND flag=0");
			if(mysqli_num_rows($q)==0){
		  		...
		  		}else{$res = 'The Requested Limit has already exist, Try with other values';}
			}if(isset($res)){$resCode='4';$resMsg=$res;}
		}elseif($chk==1){$resCode='1';$resMsg='Authentication Failed!';}
		else{$resCode='2';$resMsg='Account Locked!';}
		$result['ErrorDetails']['ErrorCode']=$resCode; $result['ErrorDetails']['ErrorMessage']=$resMsg;
...
function advances_update(){ global $mr_con;
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
	if($chk==0){
		...
		if($_REQUEST['refer']=='0'){
				...
				elseif(empty($reasonForAdv)){$res="Please Enter Remarks";}
					...
					$sql=mysqli_query($mr_con,"UPDATE ec_advances SET request_amount='$reqAmt',total_amount='$reqAmt',requested_date='$reqdate',approval_level='$level',report='$profileimg' WHERE advance_alias='$alias'");
					if($sql){$resCode = '0';$resMsg='Advance Request successfully!';}else{$resCode = '4';$resMsg='Advance Request Failed!';}
	...
	//Approver Level must be 1 and Reject
		...
		if($_REQUEST['refer']=='1'){
				...
				if($_REQUEST['ref']=='reject'){
					$limit =8;
					$sql=mysqli_query($mr_con,"UPDATE ec_advances SET approval_level='$limit',approved_by='$empalias' WHERE advance_alias='$alias'");
					...
					$sql=mysqli_query($mr_con,"UPDATE ec_advances SET approval_level='$limit',approved_by='$empalias',approved_date='$reqdate' WHERE advance_alias='$alias'");
					if($sql){ $resCode = '0';$resMsg="Approved successfully";} else {$resCode = '4';$resMsg="Approval Failed";}
					$alias_remark=aliasCheck(generateRandomString(),"ec_remarks","remark_alias");
					if($reasonForAdv!='') 
					$q=mysqli_query($mr_con,"INSERT INTO ec_remarks(remarks,module,item_alias,remarked_by,remark_alias) VALUES ('$reasonForAdv','BA','$alias','$empalias','$alias_remark')");
					}
		...
		if($_REQUEST['refer']=='2'){
				...
				if($_REQUEST['ref']=='reject'){
					...
					$sql=mysqli_query($mr_con,"UPDATE ec_advances SET approval_level='$limit',approved_by='$approved_by',approved_date='$approved_date' WHERE advance_alias='$alias'");
					if($sql){ $resCode = '0';$resMsg="Approved successfully";} else{$resCode = '4';$resMsg="Approval Failed";}
					$alias_remark=aliasCheck(generateRandomString(),"ec_remarks","remark_alias");
		...
		if($_REQUEST['refer']=='6'){
			...
			if($reasonForAdv==""){$res="Enter Remarks";}else if($utr_num==""){$res="Enter UTR Number";}else{
					$sql=mysqli_query($mr_con,"UPDATE ec_advances SET utr_num='$utr_num' WHERE advance_alias='$alias'");
					if($sql) {
						$alias_remark=aliasCheck(generateRandomString(),"ec_remarks","remark_alias");
						if($reasonForAdv!='')$q=mysqli_query($mr_con,"INSERT INTO ec_remarks(remarks,module,item_alias,remarked_by,remark_alias) VALUES ('$reasonForAdv','BA','$alias','$empalias','$alias_remark')");
...
function advances_view(){ global $mr_con;
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
	if($chk==0){
		...
		$result['submit_button'] = $submit_button;
			$remarks_sql=mysqli_query($mr_con,"SELECT remarks, remarked_by, remarked_on FROM ec_remarks WHERE item_alias='$viewemp_advalias' AND module='BA' AND flag=0");
...
function advances_mul_view(){ global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$rex=authentication($emp_alias,$token);
	if($rex==0){
		...
		if($_REQUEST['reqId']!="")$condition.="request_id LIKE '%".mysqli_real_escape_string($mr_con,$_REQUEST['reqId'])."%' AND ";
		if($_REQUEST['reqBy']!="") $condition.=" employee_alias IN (SELECT employee_alias FROM ec_employee_master WHERE name LIKE '%".$_REQUEST['reqBy']."%' AND flag=0) AND ";
		if($_REQUEST['reqDate']!="")$condition.="requested_date ='".mysqli_real_escape_string($mr_con,dateFormat(trim($_REQUEST['reqDate']),'y'))."' AND ";
		if($_REQUEST['reqAmt']!="")$condition.="request_amount LIKE '%".mysqli_real_escape_string($mr_con,$_REQUEST['reqAmt'])."%' AND ";
		if($_REQUEST['appstatus']!="")$condition.="approval_level ='".mysqli_real_escape_string($mr_con,$_REQUEST['appstatus'])."' AND ";
		$result['eadmin']=$emp_alias;
		
		if(($emp_alias=='eadmin' )|| ($emp_alias== 'EADMIN') || ($emp_alias=='admin') || ($emp_alias=='ADMIN')){
			$display_name = 'yes';
			$result['display_name']=$display_name;
			$rec=mysqli_query($mr_con,"SELECT count(id) FROM ec_advances WHERE approval_level<>'0' AND $condition flag=0");
		...
		if($chapp<='0' && ($emp_spl_dep=="1" || $emp_spl_dep=="2")){ //dep 1 for SCM,SCM PLANT & 2 for FINANACE,FINANCE-CORPORATE  
		...
		if(mysqli_num_rows($rec)>0){
			...
			if(($emp_alias=='eadmin') || ($emp_alias=='EADMIN') || ($emp_alias=='admin') || ($emp_alias=='ADMIN')){
				...
				if(in_array('5', approvelvl($emp_alias))) {
					//echo "(SELECT request_id,employee_alias,request_amount,total_amount,requested_date,approved_by,approval_level,advance_alias FROM ec_advances WHERE $condition employee_alias IN (SELECT employee_alias FROM ec_employee_master WHERE department_alias IN (SELECT approval_dep FROM ec_expense_approvals WHERE approver='$emp_alias' AND flag=0) AND flag =0) AND flag=0 AND (approval_level='5') AND employee_alias<>'$emp_alias') UNION ALL (SELECT request_id,employee_alias,request_amount,total_amount,requested_date,approved_by,approval_level,advance_alias FROM ec_advances WHERE $condition employee_alias='$emp_alias' AND flag=0) ORDER BY requested_date DESC LIMIT $offset, $limit";
					...
					//echo "SELECT request_id,employee_alias,request_amount,total_amount,requested_date,approved_by,approval_level,advance_alias FROM ec_advances WHERE $condition employee_alias='$emp_alias' AND flag=0 ORDER BY requested_date DESC LIMIT $offset, $limit";
			...
			if(mysqli_num_rows($rec)){
				$i=0;while($row = mysqli_fetch_array($rec)){ $xyz='';
					...
					$result['advancesDetails'][$i]['request_by']=alias($row['employee_alias'],'ec_employee_master','employee_alias','name');
					$result['advancesDetails'][$i]['requested_date']=dateFormat($row['requested_date'],'d');
					$result['advancesDetails'][$i]['request_amount']=$row['request_amount'];
...
function advances_export(){ global $mr_con;
set_time_limit(0);
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
	if($chk==0){
		...
		if($emp_alias !='' && $emp_alias !='0' && $emp_alias !='admin' && $emp_alias !='eadmin'){$con .= " employee_alias='$emp_alias' AND";}
		elseif($emp_alias =='admin' || $emp_alias =='eadmin'){}
		else{$result=array();
			if($arr_inter_cnt == 0){
...
function exp_dashboard(){
	...
	if($chk==0){
		...
		if($chapp>0){
			//echo "SELECT count(id) as counti FROM ec_employee_master WHERE $condition employee_alias='$emp_alias' AND flag=0) UNION (SELECT count(e.id) FROM ec_employee_master e,ec_emprole r WHERE e.role_alias = r.role_alias AND r.role_stat <> 1 AND $condition e.department_alias IN (SELECT approval_dep FROM ec_expense_approvals d WHERE d.approver='$emp_alias' AND d.flag=0) AND e.flag =0";
			$rec=mysqli_query($mr_con,"SELECT count(id) as counti FROM ec_employee_master WHERE $condition employee_alias='$emp_alias' AND flag=0 UNION SELECT count(e.id) FROM ec_employee_master e,ec_emprole r WHERE e.role_alias = r.role_alias AND r.role_stat <> 1 AND $condition e.department_alias IN (SELECT approval_dep FROM ec_expense_approvals d WHERE d.approver='$emp_alias' AND d.flag=0) AND e.flag =0");
			$tot = 0;
			//while($row=mysqli_fetch_array($rec)){$tot+=$row[0];}
			...
			if(mysqli_num_rows($sql)){
				...
				if($chapp>0){
					while($row = mysqli_fetch_array($sql)){
						...
						$result['dashboard'][$i]['total_advances']=toatlAdvances($row['employee_alias']);
						$result['dashboard'][$i]['total_expenses']=totalExpenses($row['employee_alias']);
						$result['dashboard'][$i]['reimbursement_amount']=totalReimbursement($row['employee_alias']);
						$result['dashboard'][$i]['available_balance']=advanceNotSettled($row['employee_alias']);
						$i++;
					}
				}else{
						$row = mysqli_fetch_array($sql);
						...
						$result['total_advances']=toatlAdvances($row['employee_alias']);
						$result['total_expenses']=totalExpenses($row['employee_alias']);
						$result['reimbursement_amount']=totalReimbursement($row['employee_alias']);
						$result['available_balance']=advanceNotSettled($row['employee_alias']);
...
function dashboard_empview(){ global $mr_con;
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
		if($chk==0){
			...
			$trec = 0;
			//echo "(SELECT count(id) as counti  FROM ec_advances WHERE employee_alias='$viewemp_alias' AND $condition flag=0) UNION (SELECT count(id) as counti FROM ec_expenses WHERE employee_alias='$viewemp_alias' AND $condition1 flag=0)";
		...
		if($trec>0){
			...
			if(mysqli_num_rows($sql)){
				$i=0;while($row = mysqli_fetch_array($sql)){
					...
					$result['dashempview'][$i]['amt']=$row['amt'];
					$result['dashempview'][$i]['tamt']=$row['tamt'];
					...
					if(requesttype($row['requestId'],$row['alias']) == 'advance'){
						...
						$result['dashempview'][$i]['download_page']='expenses_download';
...
function user_expences_mul_view(){
	...
	$chapp=checkApproval($emp_alias);
	$emp_spl_dep=checkspldep($emp_alias);
	if(($emp_alias=='eadmin' )|| ($emp_alias== 'EADMIN') || ($emp_alias=='admin') || ($emp_alias=='ADMIN')){$result['open_page']='';}else{
	...
	if($chk==0){
		...
		if($req_by!="")$condition.=" employee_alias IN (SELECT employee_alias FROM ec_employee_master WHERE name LIKE '%$req_by%' AND flag=0) AND ";
		...
		if($trec>0){
			...
			if(($emp_alias=='eadmin' )|| ($emp_alias== 'EADMIN')|| ($emp_alias=='admin') || ($emp_alias=='ADMIN')){
				$sql=mysqli_query($mr_con,"SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE approval_level<>'0' AND $condition flag=0 ORDER BY requested_date DESC, id DESC LIMIT $offset, $limit");
			
			}else{
				
			if($chapp=='0' && ($emp_spl_dep=="1" || $emp_spl_dep=="2")){
				$sql=mysqli_query($mr_con,"(SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition flag=0 AND approval_level<>'0' AND employee_alias<>'$emp_alias') UNION ALL (SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition employee_alias='$emp_alias' AND flag=0) ORDER BY requested_date DESC, id DESC LIMIT $offset, $limit");
			}
			else if($chapp>0){
				if (in_array('5', approvelvl($emp_alias))) {
					$sql=mysqli_query($mr_con,"(SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition employee_alias IN (SELECT employee_alias FROM ec_employee_master WHERE department_alias IN (SELECT approval_dep FROM ec_expense_approvals WHERE approver='$emp_alias' AND flag=0) AND flag =0) AND flag=0 AND approval_level='5' AND employee_alias<>'$emp_alias') UNION ALL (SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition employee_alias='$emp_alias' AND flag=0) ORDER BY requested_date DESC, id DESC LIMIT $offset, $limit");
				}
				else{
					$sql=mysqli_query($mr_con,"(SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition employee_alias IN (SELECT employee_alias FROM ec_employee_master WHERE department_alias IN (SELECT approval_dep FROM ec_expense_approvals WHERE approver='$emp_alias' AND flag=0) AND flag =0) AND flag=0 AND approval_level NOT IN('0','6','7') AND employee_alias<>'$emp_alias') UNION ALL (SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition employee_alias='$emp_alias' AND flag=0) ORDER BY requested_date DESC, id DESC LIMIT $offset, $limit");
				}
			}else{
				$sql=mysqli_query($mr_con,"SELECT id,utr_num,employee_alias,bill_number,requested_date,total_tour_expenses,places_of_visit,approval_level,expenses_alias FROM ec_expenses WHERE $condition employee_alias='$emp_alias' AND flag=0 ORDER BY requested_date DESC, id DESC LIMIT $offset, $limit");
			}
			...
			if(mysqli_num_rows($sql)){
				$i=0;while($row = mysqli_fetch_array($sql)){
					...
					$result['user_expences'][$i]['total_tour_expenses']=$row['total_tour_expenses'];
					$result['user_expences'][$i]['outbal']=advanceNotSettled($row['employee_alias']);
					$result['user_expences'][$i]['approval_level']=exlevelsName($row['approval_level']);
					$result['user_expences'][$i]['places_of_visit']=$row['places_of_visit'];
					...
					$result['user_expences'][$i]['emp_name']= employeeDetails('name',$row['employee_alias']);
	...
	if($chk==0){
		...
		if(mysqli_num_rows($oth_sql)){
			...
			while($oth_row = mysqli_fetch_array($oth_sql)){
				...
				$result['exp_others'][$i]['document_link']=$oth_row['document_link'];
		...
		$remarks_sql=mysqli_query($mr_con,"SELECT remarks, remarked_by, remarked_on FROM ec_remarks WHERE item_alias='$viewemp_expalias' AND module='BE' AND flag=0");
		$remarks_count = mysqli_num_rows($remarks_sql);
		$result['remarks']=array();
		$result['remarks_count']=$remarks_count;		
		if(mysqli_num_rows($remarks_sql)){
		...
		if (advanceNotSettled($resultz[0]['employee_alias'])!=0)$outstanding = advanceNotSettled($resultz[0]['employee_alias']); else $outstanding="No pending Advances";
		...
		$result['final_amount'] =((round($resultz[0]['total_tour_expenses'])-advanceNotSettled($resultz[0]['employee_alias'])));
	}elseif($chk==1){
		$resCode='1'; $resMsg='Authentication Failed';
	}else{$resCode='2'; $resMsg='Account Locked';}
	$result['ErrorCode']=$resCode; $result['ErrorMessage']=$resMsg;
...
function service_expences_add(){ global $mr_con;
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
				...
				if($mess==1){
					...
					for($i=0;$i<count($_REQUEST['checkin']);$i++){
						if($_REQUEST['checkin'][$i] == '' && $_REQUEST['checkout'][$i] == '' && $_REQUEST['zone_ld'][$i] == '' && $_REQUEST['state_ld'][$i] == '' && $_REQUEST['district_ld'][$i] == '' && $_REQUEST['hotelName'][$i] == '' && $_REQUEST['ticket_idld'][$i]=='' && $_REQUEST['dprNum_ld'][$i]=='' && $_REQUEST['lamt'][$i]=='')	{
							...
							if($_REQUEST['checkin'][$i]==''){$resCode='4';$mess="Lodging" .($i+1). ": Select Check in Date";$resMsg=$mess;}
							else if($_REQUEST['checkout'][$i]==''){$resCode='4';$mess="Lodging" .($i+1). ": Select Check Out Date";$resMsg=$mess;}
							//else if($_REQUEST['checkin'][$i]>=$_REQUEST['checkout'][$i]){$resCode='4';$mess="Lodging" .($i+1). ":Check Out Date should be greater than Check in Date";$resMsg=$mess;}
							else if($_REQUEST['zone_ld'][$i]=='') {$resCode='4';$mess="Lodging" .($i+1). ": Select Zone";$resMsg=$mess;}
							...
							else if($_REQUEST['hotelName'][$i]==''){$resCode='4';$mess="Lodging" .($i+1). ": Enter Hotel Name";$resMsg=$mess;}
				...
				if(isset($draft) && $draft=="draft"){
					...
					if($mr_con->query($sql)===TRUE){
						...
						if($emcheck=="NA" || $emcheck==""){
							...
							if($level==0){$resCode='0';$resMsg=" Bill Number: ".$rquestid.", Some Files could not be uploaded, So Your Expense is saved in Drafts.Kinldy Resubmit from Drafts.";}
							else {$resCode='0';$resMsg="Expense Submitted successfully with Bill Number: ".$rquestid.""; }
							//$url=baseurl()."maillings/book_expense.php?ref=".$alias;
...
function ajaxAmount(){	global $mr_con;
...
function others_expences_edit(){global $mr_con;
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
	$draft = mysqli_real_escape_string($mr_con,$_REQUEST['ref']);
	if($chk==0){
		//Draft
		 if(isset($draft) && $draft=="request"){			
			if($_REQUEST['ref2']=='1'){
					...
					if($mr_con->query($sql)===TRUE) {$resCode='0';$resMsg="Approved successfully"; }else $res="Approval Failed";
					$alias_remark=aliasCheck(generateRandomString(),"ec_remarks","remark_alias");
			...
			if($_REQUEST['ref2']=='3'){
					...
					if($reasonForAdv==""){$res="Enter Remarks";}else if($po_gnr==""){$res="Enter PO/GNR Number";}else{
					$reqdate=date("Y-m-d");
					$approved_by=expensedetlimited('approved_by',$alias)."|".$empalias;
					$approved_date=expensedetlimited('approved_date',$alias)."|".$reqdate;
			...
			if($_REQUEST['ref2']=='4'){
					...
					$reasonForAdv=mysqli_real_escape_string($mr_con,trim($_REQUEST['reasonForAdv']));
					if($reasonForAdv==""){$res="Enter Remarks";}else if($rem_amt==""){$res="Enter Reimbursement";}else{
					$reqdate=date("Y-m-d");
					$approved_by=expensedetlimited('approved_by',$alias)."|".$empalias;
					$approved_date=expensedetlimited('approved_date',$alias)."|".$reqdate;
			...
			if($_REQUEST['ref2']=='6'){
					...
					if($mr_con->query($sql)===TRUE) {$resCode='0';$resMsg="Updated successfully";} else $res="Update Failed";
					$alias_remark=aliasCheck(generateRandomString(),"ec_remarks","remark_alias");
					if($reasonForAdv!='') $mr_con->query("INSERT INTO ec_remarks(remarks,module,item_alias,remarked_by,remark_alias) VALUES ('$reasonForAdv','BE','$alias','$empalias','$alias_remark')");
					}
			}
				...
				if($mr_con->query($sql)===TRUE) {$resCode='0';$resMsg="Rejected successfully"; }else $res="Approval Failed";
				$alias_remark=aliasCheck(generateRandomString(),"ec_remarks","remark_alias");
				if($reasonForAdv!='') $mr_con->query("INSERT INTO ec_remarks(remarks,module,item_alias,remarked_by,remark_alias) VALUES ('$reasonForAdv','BE','$alias','$empalias','$alias_remark')");
...
function del_expenses(){ global $mr_con;
	...
	if($rex=='0'){
		...
		if($sqlTT){
			$update_sql = mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses = '".$diff_amt."'  WHERE expenses_alias ='".$exp_alias."' AND flag=0");
...
function expense_export(){ global $mr_con;
set_time_limit(0);
...
//echo "SELECT * FROM ec_expenses WHERE $con flag=0";
...
function del_mainexp(){ global $mr_con;
	...
	if($rex==0){
		...
		if($del_main) {$resCode='0'; $resMsg='Bill Number: '.$billno.', Successfully Deleted';}else {$resCode='1'; $resMsg='Expense Deletion Failed';}
	}elseif($rex=='1'){$resCode='1'; $resMsg='Authentication Failed';
	}else{$resCode='2'; $resMsg='Account Locked';}
	$result['ErrorDetails']['ErrorCode']=$resCode; $result['ErrorDetails']['ErrorMessage']=$resMsg;
	echo json_encode($result);
}
function advances_delete(){ global $mr_con;
	$chk=authentication($_REQUEST['emp_alias'],$_REQUEST['token']);
	if($chk==0){
	$advance_alias=mysqli_real_escape_string($mr_con,$_REQUEST['alias']);
	$rec=$mr_con->query("DELETE FROM ec_advances WHERE advance_alias='$advance_alias'");
	$mr_con->query("DELETE FROM ec_remarks WHERE item_alias='$advance_alias' AND module='BA'");
	if($rec) {$resCode = '0';$resMsg="Advance Deleted successfully";} else{$resCode = '4';$resMsg="Advance Delete Failed";}
	}
	elseif($chk==1){$resCode='1';$resMsg='Authentication Failed!';}
	else{$resCode='2';$resMsg='Account Locked!';}
	$result['ErrorDetails']['ErrorCode']=$resCode; $result['ErrorDetails']['ErrorMessage']=$resMsg;
			...
			for($i=0;$i<count($_REQUEST['amt_l']);$i++){
				...
				if($_REQUEST['zone_l'][$i] != '' && $_REQUEST['state_l'][$i] != '' && $_REQUEST['district_l'][$i] != '' && $_REQUEST['bucket'][$i] !='' && $_REQUEST['dot_l'][$i]!='' && $_REQUEST['mot_l'][$i]!='' && $_REQUEST['from_l'][$i]!='' && $_REQUEST['to_l'][$i]!='' && $_REQUEST['ticket_idl'][$i]!='' && $_REQUEST['dprNum_l'][$i]!='' && $_REQUEST['amt_l'][$i]!='')	{
					...
					if($loc_insqry){
						...
						$upval=$texp+$fd[$i];
						$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$exp_alias."'");
					}
				}
			}
			if($loc_insqry){$resCode='0';$resMsg='Inserted Successfully';}else{$resCode='4';$resMsg='Failed';}
		}
		if(isset($res)){$resCode='4';$resMsg=$res;}
	}elseif($chk=='1'){$resCode='1';$resMsg='Authentication Failed!';}
	else{$resCode='2';$resMsg='Account Locked!';}
				...
				$up=mysqli_query($mr_con,"UPDATE ec_localconveyance SET date_of_travel='".$f2."', mode_of_travel='".$f3."', from_place='".$f4."', to_place='".$f5."', amount='".$f6."', created_date='$reqdate', zone_alias='".$f7."', state_alias='".$f8."', district_alias='".$f9."', bucket='".$f11."', capacity='".$f12."', quantity='".$f14."', km='".$f15."',dpr_number='".$f16."',ticket_alias='".$f17."'
				 WHERE alias='".$loc_alias."'");
					if($up){
						...
						$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$ealias."'");
					}
...
function oth_loc_single_add(){ global $mr_con;
	...
	if($chk=='0'){
		...
		if($mess==1){
			...
			if($ins){$resCode='0';$resMsg='Inserted Successfully';}else{$resCode='4';$resMsg='Failed';}		
		}
		if(isset($res)){$resCode='4';$resMsg=$res;}
	}elseif($chk=='1'){$resCode='1';$resMsg='Authentication Failed!';}
	else{$resCode='2';$resMsg='Account Locked!';}
					...
					if($move){
						...
						if($_REQUEST['dot'][$i] != '' && $_REQUEST['mot'][$i] != '' && $_REQUEST['from'][$i] != '' && $_REQUEST['to'][$i] !='' && $_REQUEST['cticket_id'][$i]!='' && $_REQUEST['cdprno'][$i]!='' && $_FILES['motbill']['name'][$i]!='' && $_REQUEST['amt'][$i]!=''){
							...
							if($ins){
								...
								$texp=$sel_rs['total_tour_expenses'];
								$upval=$texp+$f6[$i];
								$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$con_alias."'");
							}
						}
					}else{
						array_push($fault,$_FILES['motbill']['name'][$i]);
				...
				if($_FILES['motbill']['size'][$i]>'0'){
					...
					if($move){
						...
						if($ins){
							...
							$texp=$sel_rs['total_tour_expenses'];
							$upval=$texp+$fd[$i];
							$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$alias."'");
						}
					}
					}else {array_push($fault,$_FILES['motbill']['name'][$i]);}
				}
...
// End - Conveyance
// Start - Lodging
function ser_lod_single_add(){ global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$chk=authentication($emp_alias,$token);
	if($chk=='0'){
		...
		if($mess==1){
			...
			for($i=0;$i<count($_REQUEST['lamt']);$i++){
				...
				$alias1=aliasCheck(generateRandomString(),"ec_lodging","alias");
				if($_REQUEST['checkin'][$i] != '' && $_REQUEST['checkout'][$i] != '' && $_REQUEST['zone_ld'][$i] != '' && $_REQUEST['state_ld'][$i] != '' && $_REQUEST['district_ld'][$i] != '' && $_REQUEST['hotelName'][$i] != '' && $_REQUEST['ticket_idld'][$i]!='' && $_REQUEST['dprNum_ld'][$i]!='' && $_REQUEST['lamt'][$i]!='')	{
					$ins=mysqli_query($mr_con,"INSERT INTO ec_lodging(check_in,check_out,type_of_stay,hotel_name,amount,zone_alias,state_alias,district_alias,dpr_number,expenses_alias,alias,document_link,ticket_alias,created_date) VALUES('$faa[$i]','$fa[$i]','$fb[$i]','$fc[$i]','$fd[$i]','$fe[$i]','$ff[$i]','$fg[$i]','$fh[$i]','$lod_alias','$alias1','$profileimg','$fi[$i]','$reqdate')");
					if($ins){
						$sel=mysqli_query($mr_con,"SELECT total_tour_expenses FROM ec_expenses WHERE expenses_alias = '".$lod_alias."'");
						$sel_rs=mysqli_fetch_array($sel);
						$texp=$sel_rs['total_tour_expenses'];
						$upval=$texp+$fd[$i];
						$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$lod_alias."'");
					}
				}
			}
			if($ins){$resCode='0';$resMsg="Inserted Successfully";}else{$resCode='4';$resMsg='Failed';}	
...
function lod_single_view(){	global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$chk=authentication($emp_alias,$token);
	if($chk=='0'){
...
function ser_lod_single_edit(){ global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$chk=authentication($emp_alias,$token);
	if($chk=='0'){
		if($_REQUEST['checkin']==''){$resCode='4';$mess="Lodging : Select Check In Date";$resMsg=$mess;}
		...
		else if($_REQUEST['hotelName']==''){$resCode='4';$mess="Lodging : Enter Hotel Name";$resMsg=$mess;}
		...
		if($mess==1){
			...
			$profileimg =0;
			if($f1!=''){
				$up=mysqli_query($mr_con,"UPDATE ec_lodging SET check_in='".$f2."',check_out='".$f3."',type_of_stay='".$f4."',hotel_name='".$f5."',amount='".$f6."',document_link='$profileimg',created_date='$reqdate',zone_alias='".$f7."',state_alias='".$f8."',district_alias='".$f9."',dpr_number='".$f10."',ticket_alias='".$f11."' WHERE alias='".$f1."'");
				if($up){
					$sel=mysqli_query($mr_con,"SELECT total_tour_expenses FROM ec_expenses WHERE expenses_alias = '".$ealias."'");
					$sel_rs=mysqli_fetch_array($sel);
					$texp=$sel_rs['total_tour_expenses'];
			...
			for($i=0;$i<count($_REQUEST['bamt']);$i++){
				...
				if($_REQUEST['checkinb'][$i] != '' && $_REQUEST['checkoutb'][$i] != '' && $_REQUEST['zone_bo'][$i] != '' && $_REQUEST['state_bo'][$i] != '' && $_REQUEST['district_bo'][$i] != '' && $_REQUEST['ticket_bo'][$i] != '' && $_REQUEST['dprNum_bo'][$i]!='' && $_REQUEST['bamt'][$i]!=''){
					...
					if($ins){
						...
						$texp=$sel_rs['total_tour_expenses'];
						$upval=$texp+$fd[$i];
						$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$bod_alias."'");
					}
				}
			}
			if($ins){$resCode='0';$resMsg="Inserted Successfully";}else{$resCode='4';$resMsg='Failed';}	
...
function oth_bod_single_add(){ global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$chk=authentication($emp_alias,$token);
	if($chk=='0'){
					...
					if(move_uploaded_file($_FILES["ofile"]["tmp_name"][$i],"../../attachments/".$fileName)){
						...
						if($_REQUEST['others'][$i] != '' && $_REQUEST['odate'][$i] != '' && $_FILES['ofile']['name'][$i] != '' && $_REQUEST['ticket_ot'][$i] != '' && $_REQUEST['dprNum_ot'][$i]!='' && $_REQUEST['oamt'][$i]!=''){
							...
							if($ins){
								...
								$texp=$sel_rs['total_tour_expenses'];
								$upval=$texp+$fb[$i];
								$update=mysqli_query($mr_con,"UPDATE ec_expenses SET total_tour_expenses='".$upval."' WHERE expenses_alias='".$oth_alias."'");
							}
						}
					}else{
						array_push($fault,$_FILES['ofile']['name'][$i]);
...
function expense_main_edit(){ global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$chk=authentication($emp_alias,$token);
	if($chk=='0'){
		if(!preg_match("/^(0[1-9]|[1-2][0-9]|3[0-1])-(0[1-9]|1[0-2])-[0-9]{4}$/",$_REQUEST['visitFromDate'])){$res="Select Visit from Date";}
			...
			$sql="UPDATE ec_expenses SET period_of_visit_from='$visitFromDate', period_of_visit_to='$visitToDate', places_of_visit='$placesOfVisit', purpose='$purpose', requested_date='$reqdate', approval_level='$level',report='$profileimg' WHERE expenses_alias='$ealias'";
			if($_REQUEST['ref2']=='0'){
			...
			//$mr_con->query("UPDATE ec_remarks SET remarks='$remarkss',remarked_on='".date("Y-m-d H:i:s")."' WHERE item_alias='$ealias'");
			if($mr_con->query($sql)===TRUE){$resCode='0';$resMsg='Expense Updated successfully!';}else{ $res="Expense Request Failed";}
		}
		if(isset($res)){$resCode='4';$resMsg=$res;}
	}elseif($chk=='1'){$resCode='1';$resMsg='Authentication Failed!';}
	else{$resCode='2';$resMsg='Account Locked!';}
...
function user_expense_submit(){global $mr_con;
	$emp_alias = mysqli_real_escape_string($mr_con,$_REQUEST['emp_alias']);
	$token = mysqli_real_escape_string($mr_con,$_REQUEST['token']);
	$chk=authentication($emp_alias,$token);
	...
	if($chk=='0'){	
		...
		$sql="UPDATE ec_expenses SET requested_date='$reqdate', approval_level='$level' WHERE expenses_alias='$alias'";
		if($mr_con->query($sql)===TRUE) {$resCode='0'; $resMsg="Expense Submitted successfully";} else $res="Expense Request Failed";
		//$url="http://enersyscare.co.in/enersys_expense/maillings/book_expense.php?ref=".$alias;
		//file_get_contents($url);
