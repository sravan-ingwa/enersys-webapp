...
$resultz=expensefullView($alias);
	if($resultz!=""){
		...
		$download.="<br><br><table style='padding-left:10px; line-height:1.5;'><tr><td>(".employeeDetails('name',$resultz[0]['employee_alias']).")<br><b>Signature of client</b></div><p>Date : ".dat($resultz[0]['requested_date'])."</p>";
		$download.='</td></tr></table>';
		
		if($resultz[0]['approved_by']!=""){
			$hsf = explode("|",$resultz[0]['approved_by']); $hsfDate = explode("|",$resultz[0]['approved_date']);
