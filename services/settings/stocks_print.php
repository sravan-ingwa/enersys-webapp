	...
	<table class="table1">
		<tr>
			<td class="td1">
				...
				<table class="cont_3">
					<tr>
						<td align="left"><h3>'.strtoupper("STOCKS DETAILS").'</h3></td>
					</tr>
				</table>
					<table class="cont_2">
						<tr>
							<td width="33.33%"><h3>'.strtoupper('Item Code').'</h3><p>'.checkempty(ucfirst($row['item_type']==1) ? alias($row['item_code'],'ec_product','product_alias','item_code'):alias($row['item_code'],'ec_accessories','accessories_alias','item_code')).'</p></td>
							<td width="33.33%"><h3>'.strtoupper('Item Type').'</h3><p>'.checkempty(ucfirst($row['item_type']==1) ? "CELLS" : "ACCESSORIES").'</p></td>
							<td width="33.33%"><h3>'.strtoupper('Item Description').'</h3><p>'.checkempty(strtolower($row['item_description'])).'</p></td>
						</tr>
					</table>
					<table class="cont_2">
						<tr>
							<td width="33.33%"><h3>'.strtoupper('Quantity').'</h3><p>'.checkempty(ucfirst($row['quantity'])).'</p></td>
							<td width="33.33%"><h3>'.strtoupper('Item Price').'</h3><p>'.checkempty(ucfirst($row['item_price'])).'</p></td>
							<td width="33.33%"><h3>'.strtoupper('Sjo No').'</h3><p>'.checkempty(ucfirst($row['sjo_no'])).'</p></td>
						</tr>
					</table>
					<table class="cont_2">
						<tr>
							<td width="33.33%"><h3>'.strtoupper('Invoice No').'</h3><p>'.checkempty(ucfirst($row['invoice_no'])).'</p></td>
							<td width="33.33%"><h3>'.strtoupper('Invoice Date').'</h3><p>'.checkempty(ucfirst($row['invoice_date'])).'</p></td>
