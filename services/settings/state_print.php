...
$heading = 'STATE';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_state WHERE state_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("STATE DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Zone Name').'</h3><p>'.checkempty(ucfirst(alias($row['zone_alias'],'ec_zone','zone_alias','zone_name'))).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('State Name').'</h3><p>'.checkempty(ucfirst($row['state_name'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('State Code').'</h3><p>'.checkempty(ucfirst($row['state_code'])).'</p></td>
					</tr>
				</table>';
			$print.='</td>
			</tr>
