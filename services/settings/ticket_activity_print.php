...
$heading = 'ACTIVITY';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_activity WHERE activity_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("ACTIVITY DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Activity Name').'</h3><p>'.checkempty(ucfirst($row['activity_name'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('Activity Code').'</h3><p>'.checkempty(ucfirst($row['activity_code'])).'</p></td>
						<td width="33.33%"></td>
					</tr>
				</table>';
			$print.='</td>
