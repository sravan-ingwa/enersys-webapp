...
$heading = 'SITE TYPE';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_site_type WHERE site_type_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("SITE TYPE DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Site Type Name').'</h3><p>'.checkempty(ucfirst($row['site_type'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('Segment Name').'</h3><p>'.checkempty(ucfirst(alias($row['segment_alias'],'ec_segment','segment_alias','segment_name'))).'</p></td>
						<td width="33.33%"></td>
					</tr>
				</table>';
			$print.='</td>
