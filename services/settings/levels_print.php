...
$heading = 'ACTIVITY';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_levels WHERE level_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("LEVEL DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Level Name').'</h3><p>'.checkempty(ucfirst($row['level_name'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('Level Color').'</h3><p>'.checkempty(ucfirst($row['level_color'])).'</p></td>
						<td width="33.33%"></td>
					</tr>
				</table>';
			$print.='</td>
