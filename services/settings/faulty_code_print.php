...
$heading = 'FAULTY CODE';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_faulty_code WHERE faulty_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("FAULTY CODE DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Faulty Code').'</h3><p>'.checkempty(ucfirst($row['faulty_code'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('Description').'</h3><p>'.checkempty(ucfirst($row['description'])).'</p></td>
						<td width="33.33%"></td>
					</tr>
				</table>';
			$print.='</td>
