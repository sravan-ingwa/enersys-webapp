...
$heading = 'TICKET COMPLAINT';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_complaint WHERE complaint_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("TICKET COMPLAINT DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Complaint Name').'</h3><p>'.checkempty(ucfirst($row['complaint_name'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('Activity Name').'</h3><p>'.checkempty(ucfirst(alias($row['activity_alias'],'ec_activity','activity_alias','activity_name'))).'</p></td>
						<td width="33.33%"></td>
					</tr>
				</table>';
			$print.='</td>
