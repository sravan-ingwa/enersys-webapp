...
$heading = 'DEPARTMENT';
$query = mysqli_query($mr_con,"SELECT * FROM  ec_designation WHERE designation_alias='$alias' AND flag='0'");
if(mysqli_num_rows($query)){
	...
	<table class="table1">
		<tr>
			...
			<table class="cont_3">
				<tr>
					<td align="left"><h3>'.strtoupper("DESIGNATION DETAILS").'</h3></td>
				</tr>
			</table>
				<table class="cont_2">
					<tr>
						<td width="33.33%"><h3>'.strtoupper('Designation Name').'</h3><p>'.checkempty(ucfirst($row['designation'])).'</p></td>
						<td width="33.33%"><h3>'.strtoupper('Grade').'</h3><p>'.checkempty(ucfirst($row['grade'])).'</p></td>
						<td width="33.33%"></td>
					</tr>
				</table>';
			$print.='</td>
