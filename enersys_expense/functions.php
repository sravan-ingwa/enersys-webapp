...
function baseurl() {
	return "https://enersyscare.co.in/";
}
function TitleFav(){
...
function advanceNotSettled($fv1){ 
	global $mr_con;
	$query = "SELECT SUM(request_amount) AS totalRequest FROM ec_advances WHERE employee_alias='$fv1' AND approval_level IN ('7','6') AND flag=0";
	$rec=$mr_con->query($query);
	$query = "SELECT SUM(total_tour_expenses) AS totalRequest1,SUM(reimbursement_amount) AS totalReiem, SUM(refund_amount) AS totalRefund FROM ec_expenses WHERE employee_alias='$fv1' AND approval_level IN ('7','6') AND flag=0";
	$rec1=$mr_con->query($query);
	if($rec) {
		$row = $rec->fetch_assoc();
		$row1 = $rec1->fetch_assoc();
		return (
			$row['totalRequest'] 
			- ($row1['totalRequest1']-$row1['totalReiem']) 
			- $row1['totalRefund']
		);
	} else return 0;
}
