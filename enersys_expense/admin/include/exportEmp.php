...
if(isset($_REQUEST['designation'])){$designation = implode("|",$_REQUEST['designation']);}else{$designation ='';}
if(isset($_REQUEST['department'])){$department = implode("|",$_REQUEST['department']);}else{$department = '';}
if(isset($_REQUEST['status'])){$status = implode("|",$_REQUEST['status']);}else{$status = '';}
if(!empty($designation)){$con .= " designation_alias RLIKE '$designation' AND";}
if(!empty($department)){$con .= " department_alias RLIKE '$department' AND";}
if(!empty($status)){$con .= " status RLIKE '$status' AND";}
$sql = mysql_query("SELECT * FROM ec_employee_master WHERE $con flag=0");
$colArr=array('Employee ID','Name','Email ID','Mobile Number','Joining Date','Relieving Date','Status','Grade','Designation','Department');
$colArr2=array('employee_id','name','email_id','mobile_number','joining_date','relieving_date','status');
	$filename = 'employeeDetails-'.rand(0000,9999)."-".date("Y-m-d");
	...
	while($row=mysql_fetch_array($sql)){ $coo++;
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$coo, alias($row['designation_alias'],'ec_designation','designation_alias','grade'));
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$coo, alias($row['designation_alias'],'ec_designation','designation_alias','designation'));
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$coo, alias($row['department_alias'],'ec_department','department_alias','department_name'));
	}
...
<div class="panel panel-primary">
	<div class="panel-heading">
		<h3 class="panel-title" style="display:inline-block;">Export Employee List</h3>
	</div>
...
<?php if(isset($message))echo $message;?>
        ...
        <div class="col-md-6 col-md-offset-3">
            ...
            <div class="col-md-6 form-group">
                ...
                <select name="designation[]" class="form-control"  id='mot2' multiple='multiple'placeholder="Designation"><?php echo allOptions('designation_alias','designation','ec_designation'); ?></select>
            </div>
            <div class="col-md-6 form-group">
            	<label>Levels: </label>
                <select name="status[]" class="form-control" id='mot3' multiple="multiple" placeholder="Status"><?php
                $sql = mysql_query("SELECT status FROM ec_employee_master WHERE flag=0 GROUP BY status");
                if(mysql_num_rows($sql)){while($row = mysql_fetch_array($sql)){$res .= "<option value='$row[status]'>$row[status]</option>";}echo $res;}?>
                </select>
            </div>
