...
<div class="modal-style" ng-controller="locConvy_O_EditCtrl">	
	...
	<div class="modal-body" ng-controller="updatingCntrl">
		<form class="form-horizontal forms_update" name="advanceRequest" method="post" url="services/expense_tracker/oth_loc_single_edit" ng-submit="upadteRequest()" novalidate>
           <div>
                 ...
                 <div class="row form-group">
                    <div class="col-sm-3" ng-controller="DatepickerDemoCtrl">
                        <md-input-container flex="" class="md-default-theme md-input-has-value">
                            ...
                            <input type="text" name="dot_l" readonly="readonly" value="{{editViews.date_of_travel}}" class="border-bottom" placeholder="DD-MM-YYYY" datepicker-popup="{{format}}" ng-click="open($event)" ng-focus="open($event)" ng-model="editViews.date_of_travel" is-open="opened" min-date="minDate" max-date="dt" datepicker-options="dateOptions" date-disabled="disabled(date, mode)"  show-button-bar="false"/>  
                        </md-input-container>
                    </div>
                    <div class="col-sm-3" ng-controller="locOfTravelCntrl">
                        ...
                        <select class="form-control selectdrop SlectBox" ng-model="mots" name="mot_l" >
                            ...
                            <option ng-repeat="mof in locOfTravel" value="{{mof.name}}" ng-selected="mof.name == editViews.mode_of_travel">{{mof.name}}</option>
                        </select>
                    </div>
                    <div class="col-sm-3">
                        <md-input-container flex="" class="md-default-theme md-input-has-value">
                            <label for="input_00B">From</label>
                            <input value="{{editViews.from_place}}" name="from_l">
                        </md-input-container>
                    </div>
                    <div class="col-sm-3">
                        <md-input-container flex="" class="md-default-theme md-input-has-value">
                            <label for="input_00B">To</label>
                            <input value="{{editViews.to_place}}" name="to_l">
                        </md-input-container>
                    </div>
                    <div class="col-sm-3">
                        <md-input-container flex="" class="md-default-theme md-input-has-value">
                            <label for="input_00B">Amount</label>
                            <input value="{{editViews.amount}}" name="amt_l" class="amtt tamfor ttcm amntDig"  ng-keypress="onlyIntegers($event)" ng-keyup="amnt(); onlyIntegers($event)" ng-focus="onlyIntegers($event)" autocomplete="off">
                        </md-input-container>
                    </div>
                 </div>
            </div>	
