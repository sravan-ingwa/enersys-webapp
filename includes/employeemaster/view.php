...
<div ng-controller="EmployeeMasterCtrl">
	...
	<div ng-controller="mul_view_form">
		<div class="panel panel-lined table-responsive panel-hovered mb10">
            ...
            <ol class="breadcrumb breadcrumb-small right" style="margin-bottom:0px;" ng-if="datas.export">
                <li><a href="" ng-click="employeemasterexportOpen()" class="padding-10 export-btn">Export</a></li>
            </ol>
       </div>
		<!-- row -->
		<div class="row">
            ...
            <div class="panel panel-default">
             <form class="form-horizontal forms_ec" url="services/employeemaster/employeemaster_mul_view" name="userForm" method="post" novalidate>
                <table class="table table-condensed" >
                        ...
                        <tr>
                            ...
                            <th class="hidden-xs hidden-sm" ng-controller="emproledropCntrl">
                                <select name="roleName" placeholder="Role" class="SlectBox form-control" ng-model="role" data-ng-change="listSorting()">
                           ...
                           <th class="hidden-xs">
							...
							<th class="hidden-xs hidden-sm" ng-controller="empstatusdropCntrl">
                                <select class="form-control SlectBox selectdrop" placeholder="Status" name="empStatus" ng-model="empStatus" data-ng-change="listSorting()">
                            ...
                            <th class="hidden-xs hidden-sm" ng-if="datas.delete || datas.restore">
                                Actions
                            </th>
                        </tr>
                        </thead>
                        ...
                        <table class="table table-hover">
                            <tbody>
                               <tr class="tktBackground" ng-repeat="data in datas.employeemasterDetails">
                                    ...
                                    <td><span tooltip-placement="top" ng-attr-tooltip="{{data.name.length>12 ? data.name:''}}">{{data.name | limitTo : 12}}{{data.name.length>12 ? '...':''}}</span></td>
                                    <td class="hidden-xs hidden-sm"><span tooltip-placement="top" ng-attr-tooltip="{{data.designation.length>12 ? data.designation : ''}}">{{data.designation | limitTo : 12}}{{data.designation.length>12 ? '...':''}}</span></td>
                                    <td class="hidden-xs hidden-sm"><span tooltip-placement="top" ng-attr-tooltip="{{data.zone_name.length>12 ? data.zone_name:''}}">{{data.zone_name | limitTo : 12}}{{data.zone_name.length>12 ? '...':''}}</span></td>
                                    <td class="hidden-xs hidden-sm">{{data.role_name}}</td>
                                    <td class="hidden-xs">{{data.email_id}}</td>
                                    <!--<td>{{data.mobile_number}}</td>-->
                                    <td class="hidden-xs hidden-sm">{{data.status}}</td>
                                    <td class="hidden-xs hidden-sm" ng-if="datas.delete || datas.restore">
                                      <a href="javascript:void(0)" class="ml10" tooltip="Delete" 
                                        tooltip-placement="bottom" 
                                        ng-if="data.status != 'DELETED' && datas.delete"
                                        ng-click="empMasterDeleteOpen(data);" >
                                        <span class="fa fa-delete"></span>
                                      </a>
                                      <a href="javascript:void(0)" class="ml10" tooltip="Restore" 
                                        tooltip-placement="bottom" 
                                        ng-if="data.status == 'DELETED' && datas.restore"
                                        ng-click="empMasterUnDeleteOpen(data);" >
		...
		<!-- #end row -->
    <div ng-if="datas.add"><button type="button" class="btn btn-info ion ion-plus compose-btn" tooltip="Add New" tooltip-placement="top" ng-click="employeemasteraddOpen()" md-ink-ripple></button></div>
</div> <!-- #end page-wrap -->
</div>
<div id="ticketviesw">
        ...
        <div class="sidebar-wrap text-uppercase mt47">
                <div class="group clearfix head tkt-heading2">
                    ...
                    <div class="right">
                        <div class="btn-group btn-group-sm" ng-controller="ModalDemoCtrl">
                            <a href="" class="ml10" tooltip="Edit" tooltip tooltip-placement="bottom" ng-click="employeemastereditOpen()" ng-if="singleViews.edit"><span class="fa fa-edit"></span></a>
            	...
            	<div class="panel-body clearfix freez-panel">
                    ...
                    <div class="row tkt-panel">
						<div class="col-lg-4 col-md-4 col-sm-4">
                          <h6>Cash Card</h6>
                          <span class="fnt-size-11">{{singleViews.cash_card}}</span>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-4">
                          <h6>Opening Balance</h6>
                          <span class="fnt-size-11">{{singleViews.opening_balance}}</span>
                        </div>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-4">
                          <h6>TS Notified</h6>
                          <span class="fnt-size-11">{{singleViews.ths_notified==0 ? 'HIDE' : 'SHOW'}}</span>
                        </div>
                    </div>
                    <div class="row tkt-panel">
                       <div class="col-lg-4 col-md-4 col-sm-4">
