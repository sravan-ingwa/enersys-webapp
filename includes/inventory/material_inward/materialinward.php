...
<div class="page page-ui-extras" ng-controller="ModalDemoCtrl">
	<div ng-controller="MaterialinwardCtrl">
	<div ng-controller="mul_view_form">
		<div class="panel panel-lined table-responsive panel-hovered mb10" >
			<ol class="breadcrumb breadcrumb-small left" style="margin-bottom:0px;">
				...
				<li><a href="" class="padding-10">Material Inward</a></li>
			</ol>
			<ol class="breadcrumb breadcrumb-small right" style="margin-bottom:0px;" ng-if="datas.export">
				<!--<li><a href="exports/{{singleViews.file_name}}.xlsx" ng-click="materialinwardexport()" class="padding-10 export-btn">Export</a></li>-->
				<li><a href="" ng-click="materialinwardexportOpen()" class="padding-10 export-btn">Export</a></li>
			</ol>
		</div>
		<div class="row">
			<div class="col-md-12 table-height">
				<div class="panel panel-lined table-responsive panel-hovered">
					<div class="panel panel-default">
						<form class="form-horizontal forms_ec" url="services/inventory/material_inward_multi" name="userForm" method="post" novalidate>
							<table class="table table-condensed" >
								<thead>
									<tr>
										<th>
											<a class="tktid">Ref Number&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="mrfnumber" class="droptxt1 hidden" style="display:block; border:none; color:#000;" placeholder="Type keyword" data-ng-keyup="listSorting()">
										...
										<th class="hidden-xs hidden-sm" ng-controller="DatepickerDemoCtrl">
											<a class="tktid">Date&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="mdate" class="form-control datepicker border-bottom droptxt1 hidden" placeholder="Select date.." datepicker-popup="{{format}}" ng-click="open($event)" ng-model="dt" is-open="opened" min-date="minDate" max-date="dt" datepicker-options="dateOptions" date-disabled="disabled(date, mode)"  show-button-bar="false" data-ng-focus="listSorting();open($event)"/>
											<span class="ion ion-ios-close-outline inptClose hidden" data-ng-click="listSorting()"></span>
										</th>
										<th class="hidden-xs hidden-sm" ng-show="datas.sjo_inv_head">
											<a class="tktid">Inward Number&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="inv_num" class="droptxt1 hidden" style="display:block; border:none; color:#000;" placeholder="Type keyword" data-ng-keyup="listSorting()">
											<span class="ion ion-ios-close-outline inptClose hidden" data-ng-click="listSorting()"></span>
										</th>
										<th ng-show="!datas.sjo_inv_head">
											<a class="tktid">SJO Number&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="sjonumber" class="droptxt1 hidden" style="display:block; border:none; color:#000;" placeholder="Type keyword" data-ng-keyup="listSorting()">
											<span class="ion ion-ios-close-outline inptClose hidden" data-ng-click="listSorting()"></span>
										</th>
										<th class="hidden-xs">
											<a class="tktid">From W/H&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="fwh" class="droptxt1 hidden" style="display:block; border:none; color:#000;" placeholder="Type keyword" data-ng-keyup="listSorting()">
											<span class="ion ion-ios-close-outline inptClose hidden" data-ng-click="listSorting()"></span>
										</th>
										<th class="hidden-xs">
											<a class="tktid">To W/H&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="towh" class="droptxt1 hidden" style="display:block; border:none; color:#000;" placeholder="Type keyword" data-ng-keyup="listSorting()">
											<span class="ion ion-ios-close-outline inptClose hidden" data-ng-click="listSorting()"></span>
										</th>
										<th>
											<a class="tktid">Material Value&nbsp;<span class="arrow caret"></span></a>
											<input type="text" name="mvalue" class="droptxt1 hidden" style="display:block; border:none; color:#000;" placeholder="Type keyword" data-ng-keyup="listSorting()">
											<span class="ion ion-ios-close-outline inptClose hidden" data-ng-click="listSorting()"></span>
										</th>
                                 ...
                                 <table class="table table-condensed table-hover"> 
                                    <tbody>
                                        <tr class="tktBackground" ng-repeat="data in datas.requestDetails">
                                            <td ng-click="materialinwardsingleview(data.mrf_alias)"><span tooltip-placement="top" tooltip="Click to know details of {{data.mrfnumber}}">{{data.mrfnumber}}</span></td>
                                            <td class="hidden-xs hidden-sm">{{data.dateofrequest}}</td>
                                            <td>{{data.sjonumber!='' ? data.sjonumber : '-'}}</td>
											<td class="hidden-xs"><span tooltip-placement="top" tooltip="{{data.fromwh}}">{{data.fromwh | limitTo:15}}{{data.fromwh.length>15 ? '...':''}}</span></td>
                                            <td class="hidden-xs">{{data.towh}}</td>
											<td>Rs. {{data.materialvalue | number : 2}}</td>
			<td class="hidden-xs hidden-sm" ng-if="datas.delete || datas.splEdit">
              <a href="javascript:void(0)" class="ml3" tooltip="Advance Edit" tooltip-placement="bottom" ng-click="setSettingsAlias(data.mrf_alias);materialinwardeditOpen();" ng-if="datas.splEdit">
                  <span class="fa fa-spl-edit"></span>
              </a>
              <a href="javascript:void(0)" class="ml3" tooltip="Delete" tooltip-placement="bottom" ng-click="settingsDeleteOpen('material_inward', data);" ng-if="datas.delete">
			...
			<div ng-if="datas.add">
				<button type="button" class="btn btn-info ion ion-plus compose-btn" tooltip="Addnew" tooltip-placement="top" ng-click="materialinwardaddOpen()" md-ink-ripple></button>
			</div>
		</div>
		</div>
		<div id="ticketviesw">
			<div class="site-settings ticketviesw clearfix  col-xs-6 floating-sidebar" ng-class="{'open': mInward_open}">
				<div class="sidebar-wrap text-uppercase mt46">
					<div class="group clearfix head tkt-heading2">
						<div class="left"> <span class="ion ion-close-round mr10 tktviewClose" ng-click="removeMinward()"></span> 
                        <span><strong>Material Inward Details</strong></span> </div>
						<div class="right">
						...
						<div class="btn-group btn-group-sm">
							<!--<a href="" ng-if="singleViews.admin_priv" class="ml10" tooltip-placement="bottom" tooltip="Edit" ng-click="materialinwardeditOpen();"><span class="fa fa-wrench"></span></a>-->
							<a href="services/inventory/material_inward_print.php?alias={{singleViews.mrf_alias}}" target="_blank" tooltip="Print" class="ml10" tooltip-placement="bottom"><span class="ion ion-android-print"></span></a>
							<a href="services/inventory/material_inward_download.php?alias={{singleViews.mrf_alias}}" target="_blank" tooltip="Download" class="ml10" tooltip-placement="bottom"><span class="ion ion-android-download"></span></a>
					   </div>
						</div>
					</div>
					<div class="panel-body clearfix freez-panel">
						<div class="row tkt-panel">
							...
							<div class="col-lg-4 col-md-4 col-sm-4" ng-if="singleViews.inv_num!='0' && singleViews.inv_num!=''">
								<h6>Inward Number</h6>
								<span class="fnt-size-11">{{singleViews.inv_num}}</span> </div>
							<div class="col-lg-4 col-md-4 col-sm-4">
								<h6>Total  Material Value</h6>
								<span class="fnt-size-11">Rs. {{singleViews.material_value | number : 2}}</span> </div>
							<div class="col-lg-4 col-md-4 col-sm-4" ng-if="singleViews.mrf_number!='0' && singleViews.mrf_number!=''">
								<h6>MRF Number</h6>
								<span class="fnt-size-11">{{singleViews.mrf_number}}</span> </div>
							<div class="col-lg-4 col-md-4 col-sm-4" ng-if="singleViews.sjo!='0' && singleViews.sjo!=''">
								<h6>SJO Number</h6>
								<span class="fnt-size-11">{{singleViews.sjo}}</span> </div>
							<div class="col-lg-4 col-md-4 col-sm-4" ng-if="singleViews.from_type=='1' && singleViews.ticket_id!='0' && singleViews.ticket_id!=''">
								<h6>Ticket ID</h6>
							...
							<table class="table table-condensed">
								<thead>
									<tr>
										...
										<th><a class="tktid">Item Code</a></th>
										<th><a class="tktid">Cell No/ Quanty</a></th>
							...
							<table class="table table-hover table-condensed">
								<tbody>
									<tr class="tktBackground" ng-repeat="(key,rem) in singleViews.request_items">
										<td width="7%">{{key + 1}}</td>
										<td><p tooltip-placement="top" ng-if="rem.item_type ==1">Cells</p>
