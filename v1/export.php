...
if(isset($_REQUEST['export'])){
	...
	if($id!='0'){  session_start(); $empd=employeeGetID(loginDetails($_SESSION['login_user'],"empId"));
		if($menuName=='Detail View'){ $queryx = mysql_query("SELECT * FROM $tableName WHERE empId='$empd' AND flag='0'");
...
if(!serviceAccess($loginRole,$_REQUEST['x'],'Export')){
	$query = mysql_query("SELECT * FROM ss_user_role  
	JOIN ss_menu ON ss_user_role.privilageItem=ss_menu.id
	WHERE ss_user_role.privilageType='Export' AND  ss_user_role.grantable='Yes' ORDER BY ss_menu.ordering");
	$row=mysql_fetch_array($query);
	if($row)echo "<script type='text/javascript'>window.location='index.php?x=$row[id]'</script>";
	else echo"<script type='text/javascript'>window.location='logout.php?ref=noview'</script>";
}
