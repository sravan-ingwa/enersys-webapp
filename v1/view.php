...
if(!serviceAccess($loginRole,$_REQUEST['x'],'View')){
	$query = mysql_query("SELECT * FROM ss_user_role  
	JOIN ss_menu ON ss_user_role.privilageItem=ss_menu.id
	WHERE ss_user_role.privilageType='View' AND  ss_user_role.grantable='Yes' ORDER BY ss_menu.ordering");
	$row=mysql_fetch_array($query);
                ...
                <div class="panel panel-primary">
                	<div class="panel-heading">
                        ...
                        <ul class="nav nav-pills pull-right splnav">
							<?php $loginRole=loginDetails($_SESSION['login_user'],"role");?>                        
							<?php
							...
							$rowxx=mysql_fetch_array($querycc);
							 ...
							 <?php }}?>
                                <?php if(serviceAccess($loginRole,$_REQUEST['x'],'Export')){?>
                                    ...
                                    <?php /*?><li><a href="" class="tooltips" data-toggle="confirmation" data-placement="bottom"><span class="glyphicon glyphicon-send"></span></a></li><?php */?>
                                <?php }?>
                                <?php
								if($_REQUEST['x'] != 5484 && $_REQUEST['x'] != 6643 && $_REQUEST['x'] != 8865 && $_REQUEST['x'] != 9658 && $_REQUEST['x'] != 4592 && $_REQUEST['x'] != 1764){ 
									if(serviceAccess($loginRole,$_REQUEST['x'],'Delete')){?>
										...
										<?php /*?><li><a href="#" class="tooltips" data-placement="left" data-conttent="<?php echo $_REQUEST['x'].'-'.$_REQUEST['y']; ?>" data-toggle="confirmation-popout" title="Delete"><span class="glyphicon glyphicon-trash"></span></a></li><?php */?>
                                <?php }}?>
                             <?php }?>                           
                        </ul>
