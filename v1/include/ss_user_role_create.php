	...
	if($_REQUEST['role_name']==""){$result="Enter Role Name";}
		...
		if(mysql_num_rows(mysql_query("SELECT roleName FROM ss_user_role WHERE roleName='".mysql_escape_string($_REQUEST['role_name'])."' OR roleId='$ri'"))==0){
			for($i=1; $i<=count($_REQUEST['id']); $i++){
				...
				$v = mysql_query("INSERT INTO ss_user_role(id,roleId,roleName,privilageItem,privilageType,grantable,flag) VALUES('$r','$ri','".mysql_escape_string($_REQUEST['role_name'])."','$id','Create','$create','0')");
				$w = mysql_query("INSERT INTO ss_user_role(id,roleId,roleName,privilageItem,privilageType,grantable,flag) VALUES('$r','$ri','".mysql_escape_string($_REQUEST['role_name'])."','$id','View','$view','0')");
				$x = mysql_query("INSERT INTO ss_user_role(id,roleId,roleName,privilageItem,privilageType,grantable,flag) VALUES('$r','$ri','".mysql_escape_string($_REQUEST['role_name'])."','$id','Edit','$edit','0')");
				$y = mysql_query("INSERT INTO ss_user_role(id,roleId,roleName,privilageItem,privilageType,grantable,flag) VALUES('$r','$ri','".mysql_escape_string($_REQUEST['role_name'])."','$id','Delete','$delete','0')");
				$z = mysql_query("INSERT INTO ss_user_role(id,roleId,roleName,privilageItem,privilageType,grantable,flag) VALUES('$r','$ri','".mysql_escape_string($_REQUEST['role_name'])."','$id','Export','$export','0')");
				$z = mysql_query("INSERT INTO ss_user_role(id,roleId,roleName,privilageItem,privilageType,grantable,flag) VALUES('$r','$ri','".mysql_escape_string($_REQUEST['role_name'])."','$id','Special','$special','0')");
			
			}if($v && $w && $x && $y && $z)$result="".errorMsg('ERRMSG001')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 2000 ); </script>";else $result=errorMsg('ERRMSG002');
...
<form role="form" class="ss_form" method="post" id="defaultForm">
    ...
    <div class="col-md-4 form-group">
    	<label>&nbsp;</label>
        <input type="button" tabindex="2" value="Check All" class="form-control" id="check_all"/>
    </div>
    <div class="col-md-4 form-group">
    	<label>&nbsp;</label>
        <input type="button" tabindex="3" value="Uncheck All" class="form-control" id="uncheck_all"/><br><br>
    </div>
