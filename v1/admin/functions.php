...
function loginDetails($fv){
	$query = mysql_query("SELECT displayName FROM ss_superadmin WHERE email='$fv' AND flag='0'");
	if(mysql_num_rows($query)>0){
		$row=mysql_fetch_array($query); return $row['displayName'];
	}
}
function serviceAccess($fv2,$fv3){
	$query = mysql_query("SELECT * FROM ss_user_role WHERE privilageItem='$fv2' AND privilageType='$fv3' AND  grantable='Yes'");
...
function mainMenu($fv1){
	$query = mysql_query("SELECT mainMenu FROM ss_menu WHERE id='$fv1' AND flag='0'");
	$row=mysql_fetch_array($query);
	$mainm=$row['mainMenu'];
	$query = mysql_query("SELECT * FROM ss_menu	WHERE mainMenu<>'Settings' GROUP BY mainMenu ORDER BY ordering");
	if(mysql_num_rows($query)>0){
		while($row=mysql_fetch_array($query)){ //$cou = count(subMenuForXS($row['id']));
...
function subMenuDrop(){
	$query = mysql_query("SELECT * FROM ss_menu	WHERE mainMenu='Settings' AND subMenu<>'' ORDER BY ordering");
	if(mysql_num_rows($query)>0){
		while($row=mysql_fetch_array($query)){echo "<li><a href='index.php?x=$row[id]'><p>$row[subMenu]</p></a></li>";}
	}
}
...
function zonesOptions(){
	...
	}else{echo "<option value='' disabled>No Zones...</option>";}
}
...
function  natureOfActivityOptionspl(){
	$query = mysql_query("SELECT * FROM  ss_nature_of_activity WHERE id<>'705' AND flag='0' ORDER BY activity");
	if(mysql_num_rows($query)>0){
		while($row=mysql_fetch_array($query)){echo "<option value='$row[id]'>$row[activity]</option>";}
		echo "<option value='705'>Site Visit</option>";
		echo "<option value='705'>Site Closure</option>";
	}else{echo "<option value='' disabled>Add Activity First</option>";}
}
...
function escaNameOptions(){
	...
	if(mysql_num_rows($query)>0){
		while($row=mysql_fetch_array($query)){echo "<option value='$row[escaName]'>".employeeGetName($row['escaName'])."</option>";}
...
function refname($colRef,$value,$fv1){
	...
	switch ($colRef) {
		...
		break;
		case FaultCodeDescription: $finalValue = faultyCodeGetDesc($value);break;
		case CustomerCode: 
			if(is_numeric($value)){$finalValue = customerCodeGetName($value);}
			else{$finalValue = $value;}
		break;
		case CustomerName: $finalValue = $value; break;
		case Customername: $finalValue = customerCodeGetNamex($value);
		break;
		case ProductCode: 
...
function errorMsg($err){
	switch($err){
		...
		case ERRMSG003: return "Request Already Avalilable!Please try with other value";break;
		case ERRMSG004: return "Only .jpg, .jpeg, .gif, .png";break;
		case ERRMSG0041: return "Request Successful! &amp; Sent for Zonal Verification";break;
		case ERRMSG005: return "Request Successful! And Ticket Rejected";break;
		case ERRMSG006: return "Request Successful! And Sent For National Head service Verification";break;
		case ERRMSG007: return "Request Successful! And National Head service Verification Not Required";break;
...
function onlineTTMail($fv){
	/*date_default_timezone_set("Asia/Kolkata");
	$dt = date("d-m-Y H:i:s");
	$to =  "service@enersys.co.in";
	$ccemail= "ticket@enersys.co.in";
	$sub = "Open Form TT Request ".$dt;
	$body="<p>Dear Service Team,</p>";
	$body.="<p>A TT Request with ".$fv." Site Name is placed in Enersys Care at ".$dt.". Kindly close the pending request to generate the TT Number.</p>";
	$body.="<p style='font-style:italic;font-size:12px;'>*** This is a System generated email, Please do not reply ***</p>";
	$from="feedback@enersys.co.in";
	$headers="From: EnerSys Care <$from>\r\n";
	$headers.="Reply-To: $from\r\n";
