...
$query=mysql_query("SELECT * FROM ss_circles");
if(mysql_num_rows($query)>0){
while($row=mysql_fetch_array($query)){$z=zoneGetName($row['zone']);
	$result.="<tr>";
	$result.="<td>$row[id]</td>";
	$result.="<td>$z</td>";
	$result.="<td>$row[circle]</td>";
	$result.="<td>$row[description]</td>";
	$result.="<td>$row[flag]</td>";
	$result.="</tr>";
}
}else{$result="<tr><td  colspan='5'>No results</td></tr>";}
?>
<!DOCTYPE html>
<html lang="en">
  <head>
