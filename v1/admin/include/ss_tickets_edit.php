...
if(isset($_REQUEST['update'])){
	...
	if($_REQUEST['checkStat']=='0'){
		if(isset($_REQUEST['serviceEngineer']) && $_REQUEST['serviceEngineer'] != ""){
	...
	elseif($_REQUEST['checkStat']=='1' && natureOfActivityGetCode($_REQUEST['natureOfActivity'])=='AT'){
		...
		$cDate=mysql_escape_string($_REQUEST['cDate']);
		if($a==""){$result="Select Number of Banks";}
		elseif($b==""){$result="Enter Complete Observation";}
		elseif($cd==""){$result="Enter FSR Number";}
		elseif($cDate==""){$result="Select Closing Date";}
		elseif(empty($_FILES['closedfsrreport']['name'])){$result="Select Closing FSR Report";}
		elseif(empty($_FILES['sitePhotoGraphs']['name'])){$result="Select Site Photographs";}
		else{
			$sitefileName="spg".nameing($_REQUEST['y']).".pdf";
			$moveSitePhoto = move_uploaded_file($_FILES["sitePhotoGraphs"]["tmp_name"],"../reports/sitePhotoGraphs/".$sitefileName);
			$sitePhoto = "reports/sitePhotoGraphs/".$sitefileName;
			...
			if($moveSitePhoto && $moveCfsr){
				$ac = mysql_query("UPDATE ss_tickets SET closedfsrreport='$profileimg', sitePhotoGraphs='$sitePhoto', noofBanks='$a', completeobservation='$b',fsrNumber='$cd',mfgDate='$mfgDate', closingDate='$cDate', checkStat='2', ticketStatus='Visited', errorMessage='".ttMsg('3')."' WHERE id='".$_REQUEST['y'] ."'");
				if($ac){$result="".errorMsg('ERRMSG0041')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 1000 ); </script>";}
	...
	elseif($_REQUEST['checkStat']=='1' && natureOfActivityGetCode($_REQUEST['natureOfActivity'])=='I&C'){
		...
		elseif(empty($_FILES['sitePhotoGraphs']['name'])){$result="Select Site Photographs";}
			...
			$sitePhoto="spg".nameing($_REQUEST['y']).".pdf";
			$moveSitePhoto = move_uploaded_file($_FILES["sitePhotoGraphs"]["tmp_name"],"../reports/sitePhotoGraphs/".$sitePhoto);
			$sitePhotograph = "reports/sitePhotoGraphs/".$sitePhoto;
			$fileName="cfsr".nameing($_REQUEST['y']).".pdf";
			$moveCfsr = move_uploaded_file($_FILES["closedfsrreport"]["tmp_name"],"../reports/closedfsrreport/".$fileName);
			$profileimg = "reports/closedfsrreport/".$fileName;
			if($moveSitePhoto && $moveCfsr){
				$ac = mysql_query("UPDATE ss_tickets SET sitePhotoGraphs='$sitePhotograph', closedfsrreport='$profileimg', noofBanks='$a', completeobservation='$b',fsrNumber='$cd',mfgDate='$mfgDate',closingDate='$cDate', checkStat='2', ticketStatus='Visited', errorMessage='".ttMsg('3')."'  WHERE id='".$_REQUEST['y'] ."'");
				if($ac)$result="".errorMsg('ERRMSG001')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 1000 ); </script>";else $result=errorMsg('ERRMSG002');
			}else{$result="Error in uploading files, try again";}
		}
	}
		...
		if(isset($_REQUEST['fsraccept'])){
			...
			if($cvc==""){$result="Enter Comments";}
				...
				if($_REQUEST['fsraccept']==0){
					...
					if($ac)$result="".errorMsg('ERRMSG005')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 1000 ); </script>";else $result=errorMsg('ERRMSG005');	
				}
				elseif($_REQUEST['fsraccept']==1 && (natureOfActivityGetCode($_REQUEST['natureOfActivity'])=='AT' || natureOfActivityGetCode($_REQUEST['natureOfActivity'])=='I&C')){
					...
					$ac = mysql_query("UPDATE ss_tickets SET checkStat='5',errorMessage='".ttMsg('9')."', ticketStatus='Closed',closingDate='".date('Y-m-d H:i:s')."', tat='$tat', errorAdmin='$cvc', mailStat='1' WHERE id='".$_REQUEST['y'] ."'");
					if($ac){
					/* Ticket closed SMS Function */
						$numberx=mysql_escape_string($_REQUEST['CrNumx']);
				...
				elseif($_REQUEST['fsraccept']==1 && $_REQUEST['tReq']==1){
					...
					if($ac)$result="".errorMsg('ERRMSG006')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 1000 ); </script>";else $result=errorMsg('ERRMSG006');	
				}
				/*elseif($_REQUEST['fsraccept']==1 && $_REQUEST['tReq']==0){
					...
					if($ac)$result="".errorMsg('ERRMSG007')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 1000 ); </script>";else $result=errorMsg('ERRMSG007');	
				}*/
				elseif($_REQUEST['fsraccept']==1 && $_REQUEST['tReq']==2){
					...
					/* Ticket closed SMS Function Close */
					$result="".errorMsg('ERRMSG009')."<script>setTimeout(function(){ document.location = 'index.php?x=".$_REQUEST['x']."';}, 1000 ); </script>";
					}else $result=errorMsg('ERRMSG007');
				}
			}
		}
	}
...
<form role="form" class="ss_form" method="POST" name="myform" action="" enctype="multipart/form-data" id="defaultForm">
  ...
  <?php /*?>***Ticket Inactive Chechstat 0***<?php */?>
  ...
  <div class="col-md-4 form-group falhid">
    ...
    <input tabindex="13" class="form-control" type="file" name="sitePhotoGraphs"/>
  </div>
  <div class="col-md-4 form-group falhid">
    <label>FSR Number : </label>
    <input tabindex="14" class="form-control" type="text" id="fsrNumber" name="fsrNumber" value="<?php echo $row['fsrNumber']; ?>"/>
  <small id="fsrNumberError" class="help-block" style="color:#a94442;"></small>
  </div>
  <div class="col-md-4 form-group falhid">
    <label>FSR Report Upload : </label>
    <input tabindex="15" class="form-control" type="file" name="closedfsrreport"/>
  </div>
  <div class="col-md-4 form-group falhid">
    <label>Closing Remarks : </label>
    <textarea tabindex="16" class="form-control" name="completeobservation"><?php echo $row['completeobservation']; ?></textarea>
  </div>
  <?php }?>
  <?php /*?>***AT Checkstat 1***<?php */?>
  <?php /*?>***I&C Checkstat 1***<?php */?>
  ...
  <?php }?>
  <?php /*?>***I&C Checkstat 1***<?php */?>
  <?php /*?>***WS visiting Checkstat 1***<?php */?>
  <?php if($row['checkStat']=='1'&& natureOfActivityGetCode($row['natureOfActivity'])=='WS'){ ?>
  <div class="col-md-4 form-group falhid">
    <label>Visited On : </label>
    <input name="visitedby" type='text' class="form-control singleDateEnd" required="required" placeholder="YYYY-MM-DD" tabindex="12" contenteditable="false" value="<?php echo date('Y-m-d'); ?>" />
  </div>
  <div class="col-md-4 form-group falhid">
  ...
  <div class="col-md-4 form-group falhid">
    ...
    <select tabindex="19" class="form-control" name="noofworkingmodules">
      ...
      <?php for($i=1;$i<=20;$i++){ echo "<option ";if($row['noofworkingmodules']!="")echo checkstate($row['noofworkingmodules'],$i); echo " value='".$i."'>".$i."</option>"; } ?>
    </select>
  </div>
  <div class="col-md-4 form-group falhid">
  ...
  <div class="col-md-4 form-group">
    ...
    <input tabindex="11" type="text" name="falset" class="form-control" >
    <br/>
  </div>
  <?php }?>
  <?php /*?>***Rejecting of false ticket***<?php */?>
  ...
  <div class="col-md-4 form-group">
    ...
    <select name="fsraccept" id="fsraccept" data='tReq' class="form-control" onchange="setSelectBoxByText(this.getAttribute('data'), this.options[this.selectedIndex].text);">
  ...
  <div class="col-md-4 form-group">
    <label>Closing Remarks: </label>
    <textarea tabindex="41" class="form-control" name="completeobservation"></textarea>
  </div>
  <?php }?>
  <?php /*?>***WS Closing***<?php */?>
