...
if(isset($_REQUEST['update'])){
	...
	$u=strtolower(mysql_escape_string($_POST['clusterManagerMail']));	$v=ucfirst(mysql_real_escape_string($_POST['siteAddress']));
		if($k=="" && $l==""){$result="Select Manufactured Date OR Installation Date";}
		else{
			$RefId =$_REQUEST['y'];
			$date= date('Y-m-d'); 
...
<input type="hidden" name="y" value="<?php echo $RefId;?>" />
    ...
    <div class="col-md-4 form-group">
        ...
        <option value="">Select Product Code</option><?php explodeEdit($row['productCode'],'ss_product_details','productCode');?>
		</select>
    </div>
