...
if(isset($_REQUEST['update'])){
	...
	elseif($b==""){$result="Enter Description";}
			...
			$ac = mysql_query("UPDATE ss_fault_code SET fault = '".$a."',description='".$b."' WHERE id='$RefId'");
			if($ac)$result="".errorMsg('ERRMSG001')." ";else $result=errorMsg('ERRMSG002');
	}
}
$RefId =$_REQUEST['y'];
$query1=mysql_query("SELECT * FROM ss_fault_code WHERE id='$RefId'");
...
<input type="hidden" name="y" value="<?php echo $RefId;?>" />
    <div class="col-md-4 form-group">
        ...
        <input tabindex="1" autofocus="autofocus" class="form-control fulcap" type="text" name="fault" placeholder="Enter Fault Code" value="<?php echo $row['fault']; ?>">
    </div>
    <div class="col-md-4 form-group">
        <label>Description</label>
        <input tabindex="2" type="text" class="form-control" name="description" placeholder="Enter Description" value="<?php echo $row['description']; ?>">
    </div>
