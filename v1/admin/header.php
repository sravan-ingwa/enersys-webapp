	...
	<div class="row">
    	<div class="col-lg-12 topmenuc">
        	<ul class="menu-lh hidden-xs">
                <div class="testBody">
                    <li class="testRow">
                       <?php subMenu($_REQUEST['x']);?>
                    </li>
                </div>
            </ul>
            <?php if(isset($_REQUEST['x'])){ ?>
            	<ul class="menu-rh"><?php $path=basename($_SERVER['PHP_SELF'],'.php');
				if($_REQUEST['x'] == 5484 || $_REQUEST['x'] == 1764){ ?>  
					<li <?php if($path=="index"||$path=="view") echo 'class="active"'; ?> ><a href="index.php?x=<?php echo $_REQUEST['x']; ?>" title="View"><span class="glyphicon glyphicon-eye-open hidden-xs"></span><p>View</p></a></li>
                    <li <?php if($path=="export") echo 'class="active"'; ?>><a href="export.php?x=<?php echo $_REQUEST['x'];?>" title="Export"><span class="glyphicon glyphicon-export hidden-xs"></span><p>Export</p></a></li>
                <?php }else{ ?>
					<li <?php if($path=="index"||$path=="view") echo 'class="active"'; ?> ><a href="index.php?x=<?php echo $_REQUEST['x']; ?>" title="View"><span class="glyphicon glyphicon-eye-open hidden-xs"></span><p>View</p></a></li>
                    <li <?php if($path=="create") echo 'class="active"'; ?>><a href="create.php?x=<?php echo $_REQUEST['x'];?>" title="Add"><span class="glyphicon glyphicon-plus hidden-xs"></span><p>Add New</p></a></li>
                    <li <?php if($path=="export") echo 'class="active"'; ?>><a href="export.php?x=<?php echo $_REQUEST['x'];?>" title="Export"><span class="glyphicon glyphicon-export hidden-xs"></span><p>Export</p></a></li>					
				<?php if($_REQUEST['x'] == 8324 || $_REQUEST['x'] ==9272){ ?>
					<li <?php if($path=="import") echo 'class="active"'; ?>><a href="import.php?x=<?php echo $_REQUEST['x'];?>" title="Import"><span class="glyphicon glyphicon-import hidden-xs"></span><p>Import</p></a></li>
					<?php } ?>
				<?php } ?>
                </ul>
			<?php } ?>
